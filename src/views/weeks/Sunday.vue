        ...
        <v-col>
          <v-card color="#EEEEEE">
            <v-toolbar
              ...
              flat
            >
              <v-toolbar-title>タスク(日)</v-toolbar-title>
            </v-toolbar>
            <v-container>
              <v-row dense>
                <v-col
...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      ...
      this.$store.dispatch('tasks/createTaskAction', { newTask: newTask, week: 'sunday', time: time, user_id: user_id })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      if(Object.keys(tasks).length) {
        return tasks.filter(task => !task.is_done && task.week == "sunday" && task.user_id == user_id)
      }
    },
    notDoneTasks() {
      const tasks = this.tasks
