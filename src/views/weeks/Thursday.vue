...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      ...
      this.$store.dispatch('tasks/createTaskAction', { newTask: newTask, week: 'thursday', time: time, user_id: user_id })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      if(Object.keys(tasks).length) {
        return tasks.filter(task => !task.is_done && task.week == "thursday" && task.user_id == user_id)
      }
    },
    notDoneTasks() {
      const tasks = this.tasks
