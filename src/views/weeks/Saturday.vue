<template>
  <v-container>
    <v-row>
      <v-col>
        <v-card flat>
          <v-card-text class="title">タスク(土)</v-card-text>
          <v-row dense>
            <v-col
...
export default {
  ...
  methods: {
    async createTask(newTask, time) {
      ...
      await this.$store.dispatch('tasks/createTaskAction', { newTask: newTask, week: "saturday", time: time, user_id: user_id })
      this.$store.dispatch('tasks/getTasksAction')
    },
    ...
    getTaskList() {
      ...
      const user_id = this.user_id
      if (tasks.length) {
        return tasks.filter(task => task.week == "saturday" && task.user_id == user_id)
      }
    }
  },
  filters: {
