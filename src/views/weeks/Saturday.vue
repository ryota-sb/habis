        ...
        <v-col>
          <v-card color="#EEEEEE">
            <v-toolbar
              ...
              flat
            >
              <v-toolbar-title>タスク(土)</v-toolbar-title>
            </v-toolbar>
            <v-container>
              <v-row dense>
                <v-col
...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      ...
      this.$store.dispatch('tasks/createTaskAction', { newTask: newTask, week: 'saturday', time: time, user_id: user_id })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      const user_id = this.user_id
      if (tasks.length) {
        return tasks.filter(task => !task.is_done && task.week == "saturday" && task.user_id == user_id)
      }
    },
    notDoneTasks() {
      const tasks = this.tasks.data
