...
export default {
  ...
  methods: {
    createTask(newTask, time) {
      ...
      this.$store.dispatch('tasks/createTaskAction', { newTask: newTask, week: 'tuesday', time: time, user_id: user_id })
    },
    updateTask(task_id) {
    ...
    doneTasks() {
      ...
      const user_id = this.user_id
      if (tasks.length) {
        return tasks.filter(task => !task.is_done && task.week == "tuesday" && task.user_id == user_id)
      }
    },
    notDoneTasks() {
      const tasks = this.tasks.data
